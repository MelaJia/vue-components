...
function submit () {
  ...
  postDataBase.apply(this, ['/supplierOrderLoan/supplierOrderCompleteSigningDiscount.do', { loanId: this.detailsP.loanId }, true]).then(res => {
    // 操作成功 关闭弹窗
    if (res.data.status) {
...
function reject () {
  ...
  //   return
  // }
  postDataBase.apply(this, ['/supplierOrderLoan/cancelSigningDiscount.do', { loanId: this.detailsP.loanId }, true]).then(res => {
    // 操作成功 关闭弹窗
    if (res.data.status) {
      this.handleClose() // 关闭弹窗
      this.$parent.fresh() // 刷新数据
